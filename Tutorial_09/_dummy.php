<?php
    ...
    for ($i = 0; $i < $num_data; $i++) {
        ...
        if (mysqli_num_rows($result) == 0) {
            $query = "INSERT INTO `posts` (`title`, `content`, `is_published`, `created_datetime`, `updated_datetime`) VALUES ('$title', '$content', $is_published, '$created_datetime', '$updated_datetime')";
            $result = mysqli_query($conn, $query);
            if (!$result) {
